...
class Raw_Data_validation:
    ...
    def __init__(self, path):
        # Initialize with the batch directory path and schema path
    ...
    def createDirectoryForGoodBadRawData(self):
        ...
        """
        try:
            # Create Good_Raw directory if it doesn't exist
            path = os.path.join("Training_Raw_files_validated/", "Good_Raw/")
            if not os.path.isdir(path):
                os.makedirs(path)
            # Create Bad_Raw directory if it doesn't exist
    ...
    def validationFileNameRaw(self, regex, LengthOfDateStampInFile, LengthOfTimeStampInFile):
        ...
        """
        # Delete existing Good/Bad Raw folders to start fresh
        self.deleteExistingBadDataTrainingFolder()
        self.deleteExistingGoodDataTrainingFolder()

        onlyfiles = [f for f in listdir(self.Batch_Directory)]
        try:
            # Create new directories for Good and Bad Raw data
            ...
            for file in listdir('Training_Raw_files_validated/Good_Raw/'):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        ...
                        break
                if count == 0:
                    # Rename first column if missing
